...
const SingleItem = ({ item }: { item: ProductType }) => {
  ...
  const isInwishlist = wishlistItems.some(each => each.id === item.id);
      ...
      <div className="relative overflow-hidden rounded-lg bg-[#F6F7FB] min-h-[403px]">
        ...
        <div className="absolute right-0 bottom-0 translate-x-full u-w-full flex flex-col gap-2 p-5.5 ease-linear duration-300 group-hover:translate-x-0">
          ...
          <button
            ...
            id="addCartOne"
            className={`flex items-center justify-center w-9 h-9 rounded-full shadow-1 ease-out duration-200  hover:text-white hover:bg-blue relative ${isInCart ? "bg-blue text-white " : "text-dark bg-white"}`}
          >
            {
              isInCart ? <svg className="absolute right-1 top-1" width="10" height="7" viewBox="0 0 8 5" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M7.29707 0.1997C7.56334 0.465966 7.56334 0.89767 7.29707 1.16394L3.66071 4.8003C3.39828 5.06273 2.97414 5.06706 2.70641 4.81004L0.888227 3.06458C0.616583 2.8038 0.607775 2.37219 0.868553 2.10055C1.12933 1.8289 1.56094 1.82009 1.83259 2.08087L3.16885 3.36368L6.33283 0.1997C6.5991 -0.0665666 7.0308 -0.0665666 7.29707 0.1997Z" fill="white"></path>
            </svg> : null
            }
